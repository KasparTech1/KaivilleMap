...
`;
  ...
  const submitAsArticle = async () => {
    ...
    try {
      // Just queue the article for processing - don't do the heavy lifting here
      const queueResponse = await fetch('/api/research/queue-article', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            ...
            model: selectedModel,
            modelFullName: models.find(m => m.id === selectedModel)?.fullName || selectedModel.toUpperCase(),
            promptSegments,
      ...
      if (!queueResponse.ok) {
        const error = await queueResponse.json().catch(() => ({ message: 'Unknown error' }));
        throw new Error(error.error?.message || error.message || 'Failed to queue article');
      }
      
      const result = await queueResponse.json();
      console.log('Article queued for processing:', result);
