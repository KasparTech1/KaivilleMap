...
// POST /api/research/queue-article - Queue research for article conversion
async function queueArticleHandler(req, res) {
  try {
    const { content, metadata } = req.body || {};
    ...
    // Quick validation and queue the task
    const queueId = `queue_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    // Save to a queue table or temporary storage
    const { data: queueEntry, error: queueError } = await supabase
      .from('research_article_queue')
      .insert({
        id: queueId,
        content,
        metadata,
    ...
    if (queueError) {
      // If queue table doesn't exist, create a simpler response
      console.log('Queue table not found, using direct processing');
      ...
      process.nextTick(async () => {
        try {
          ...
          const title = lines[0]?.replace(/^#+\s*/, '').replace(/^THINKING PROCESS:.*$/i, 'AI Research Report').trim() 
            || `AI Research: ${metadata.assembledPrompt?.substring(0, 50)}...`;
          
          // Create article content
          const articleContent = `---
title: ${title}
subtitle: Generated by ${metadata.modelFullName} AI Research System
source_type: AI Research
domains: [AI Research, ${metadata.promptSegments?.business_unit || 'General'}, ${metadata.promptSegments?.research_domain || 'Analysis'}]
topics: [${metadata.promptSegments?.analysis_method || 'Research'}, ${metadata.promptSegments?.output_format || 'Report'}]
---

# Research Parameters

${metadata.assembledPrompt || 'Direct research request'}

# AI Model Used

**Model:** ${metadata.modelFullName}
...
${content}`;
          ...
          // Submit to paste endpoint
          await supabase
            .from('research_uploads')
            .insert({ 
              user_id: null, 
              processing_status: 'queued',
              metadata: { source: 'research_prompt_builder', ...metadata }
            });
            
          // Submit the article content directly
          const normalizeAndValidate = require('./normalizer').normalizeAndValidate;
          const createEmbeddingsForArticle = require('./embeddings').createEmbeddingsForArticle;
          ...
          if (norm.ok) {
            ...
            const { data: article } = await supabase
              ...
              .insert({
                ...
                subtitle: norm.subtitle,
                source_type: 'AI Research',
                authors: [metadata.modelFullName],
            ...
            if (article) {
              ...
              createEmbeddingsForArticle(article).catch(console.error);
              console.log('Article created in background:', article.id, article.title);
            }
          }
        } catch (bgError) {
          console.error('Background article processing failed:', bgError);
        }
      });
    }
    ...
    console.error('Queue article error:', e);
    return serverError(res, e.message);
  }
}
